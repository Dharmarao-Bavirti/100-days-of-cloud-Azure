...
## Create a Virtual Machine
   ...
   - Create a new Virtual Machine named `nautilus-vm` in the `westus` region.
...
1. Ensure that the SSH key from the `azure-client` host is added to the azureuser account on `nautilus-vm`.
   - Copy `id_rsa.pub` from `azure client` to `nautilus-vm` `authorized_keys` of user `azure-user`.
2. Enable secure, password-less SSH access from the `azure-client` host.

   ```bash
   ssh azureuser@<nautilus-vm-public-key>
   ```
